...
CHECK_C11_C99_COMPILER_FLAG(MCR_C11_CFLAG)

# Check what flag is needed for C++ 2011 support.
INCLUDE(CheckCXX11CompilerFlag)
CHECK_CXX11_COMPILER_FLAG(MCR_CXX11_CXXFLAG)

# Disable C++ RTTI.
INCLUDE(CheckCXXNoRTTICompilerFlag)
CHECK_CXX_NO_RTTI_COMPILER_FLAG(MCR_CXX_NO_RTTI_CXXFLAG)

# Disable C++ exceptions.
INCLUDE(CheckCXXNoExceptionsCompilerFlag)
CHECK_CXX_NO_EXCEPTIONS_COMPILER_FLAG(MCR_CXX_NO_EXCEPTIONS_CXXFLAG)

SET(MCR_C_FLAGS_COMMON "${MCR_C11_CFLAG}")
SET(MCR_CXX_FLAGS_COMMON "${MCR_CXX11_CXXFLAG} ${MCR_CXX_NO_RTTI_CFLAG} ${MCR_CXX_NO_EXCEPTIONS_CFLAG}")
SET(MCR_EXE_LINKER_FLAGS_COMMON "")

UNSET(MCR_C99_CFLAG)
UNSET(MCR_CXX11_CXXFLAG)
UNSET(MCR_CXX_NO_RTTI_CFLAG)
...
SET(FLAG_TEST "-O")
	...
	IF(LDFLAG_${FLAG_TEST})
		SET(MCR_EXE_LINKER_FLAGS_COMMON "${MCR_EXE_LINKER_FLAGS_COMMON} -Wl,-O1")
	ENDIF(LDFLAG_${FLAG_TEST})
UNSET(FLAG_TEST)
...
SET(FLAG_TEST "--compress-debug-sections")
	...
	IF(LDFLAG_${FLAG_TEST})
		SET(MCR_EXE_LINKER_FLAGS_COMMON "${MCR_EXE_LINKER_FLAGS_COMMON} -Wl,${FLAG_TEST}=zlib")
	ENDIF(LDFLAG_${FLAG_TEST})
UNSET(FLAG_TEST)
...
# Debug/release flags.
SET(MCR_C_FLAGS_DEBUG		"${CFLAG_OPTIMIZE_DEBUG} -ggdb -DDEBUG -D_DEBUG")
SET(MCR_CXX_FLAGS_DEBUG		"${CFLAG_OPTIMIZE_DEBUG} -ggdb -DDEBUG -D_DEBUG")
SET(MCR_C_FLAGS_RELEASE		"-O2 -ggdb -DNDEBUG")
SET(MCR_CXX_FLAGS_RELEASE	"-O2 -ggdb -DNDEBUG")
...
IF(ENABLE_LTO)
	...
	IF(EXISTS "${GCC_WRAPPER_AR}" AND EXISTS "${GCC_WRAPPER_RANLIB}")
		...
		IF(CFLAG_LTO)
			SET(MCR_C_FLAGS_RELEASE   "${MCR_C_FLAGS_RELEASE} -flto")
			SET(MCR_CXX_FLAGS_RELEASE "${MCR_CXX_FLAGS_RELEASE} -flto")
			SET(MCR_EXE_LINKER_FLAGS_RELEASE    "${MCR_EXE_LINKER_FLAGS_RELEASE} -flto -fuse-linker-plugin")
			SET(MCR_SHARED_LINKER_FLAGS_RELEASE "${MCR_SHARED_LINKER_FLAGS_RELEASE} -flto -fuse-linker-plugin")
			SET(MCR_MODULE_LINKER_FLAGS_RELEASE "${MCR_MODULE_LINKER_FLAGS_RELEASE} -flto -fuse-linker-plugin")
		ELSE(CFLAG_LTO)
